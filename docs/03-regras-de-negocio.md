...
| RN-078 | Aplicar um **preset** copia seus valores para o nível escolhido (Geral ou Empreendimento) **sem criar vínculo**: alterações futuras no preset não afetam quem já o aplicou. Depois de aplicado, qualquer valor pode ser sobrescrito normalmente. |
| RN-079 | **Defaults conservadores**: sem configuração, o sistema **não** aplica juros nem reajuste (método e índice = `nenhum`) e **não** envia mensagens de cobrança. Parâmetros comerciais/jurídicos críticos (retenção de distrato, percentual de comissão, modelo jurídico) **não têm default** e devem ser configurados antes da primeira venda do empreendimento. |
...
| RN-031a | **Caso real (ESW), confirmado**: **sem juros mensais**; **reajuste anual = IGP-M acumulado (12m) + 1% fixo (aditivo)**, a partir do 13º mês (carência de 12 meses); **índice acumulado negativo conta como zero** (o +1% se mantém). A premissa anterior de “1% ao mês” foi **descartada**. Os parâmetros permitem também padrões de mercado (Price/SAC com juros mensais, correção mensal). *(ver [`07`](07-perfil-cliente-esw.md) e o motor [`05`](05-motor-calculo-financeiro.md))* |
